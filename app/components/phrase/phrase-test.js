...
describe('aacApp.phrase module', () => {
    ...
    describe('phrase controller', () => {
        it('should be defined', () => {
            phraseCtrl = $controller(PhraseController, {
                $scope: {},
                $global: g,
                $timeout: $timeout,
                EventManager: event,
    ...
    describe('phraseTouchStart', () => {
        it('should not activateDraggable if it is scrolled', () => {
            g.currentPhrase = generateSymbol(2, 1, 1, 1);
            phraseCtrl = $controller(PhraseController, {
                $scope: {},
                $global: g,
            ...
            let cpDOM = document.createElement("div");
            document.body.appendChild(cpDOM);
            cpDOM.innerHTML = "<div id='cpDOMInner'>Deneme</div>";
            cpDOM.style.width = "900px";
            cpDOM.style.overflow = "scroll";
            let cpDOMInner = document.getElementById("cpDOMInner");
            cpDOMInner.style.width = "10000px";
            cpDOM.scrollLeft = "10";
            phraseCtrl.phraseTouchStart({
                touched: [{
                    clientX: 0
                }]
            }, cpDOM);
            cpDOM.scrollLeft = "300";
            $timeout.flush();
            // Flush 1000ms timeout.

            expect(cpDOM.scrollLeft)
                .toBe(300);
            expect(g.isDraggable)
                .toBe(true);
        });
        it('should activateDraggable if it is not scrolled', () => {
            g.currentPhrase = generateSymbol(2, 1, 1, 1);
            phraseCtrl = $controller(PhraseController, {
                $scope: {},
                $global: g,
            ...
            let cpDOM = document.createElement("div");
            document.body.appendChild(cpDOM);
            cpDOM.innerHTML = "<div id='cpDOMInner2'>Deneme</div>";
            cpDOM.style.width = "900px";
            cpDOM.style.overflow = "scroll";
            let cpDOMInner = document.getElementById("cpDOMInner2");
            cpDOMInner.style.width = "10000px";
            cpDOM.scrollLeft = "10";
            console.log(cpDOM.scrollLeft);
            phraseCtrl.phraseTouchStart({
                touched: [{
                    clientX: 0
                }]
            ...
            expect(cpDOM.scrollLeft)
                .toBe(10);
            expect(phraseCtrl.activatedFirstTime)
                .toBe(true);
        });
    });
